...
function installOpenVPN() {
	...
	if [[ ! -d /etc/openvpn/easy-rsa/ ]]; then
		...
		# Generate a random, alphanumeric identifier of 128 characters for CN and one for server name
		SERVER_CN="cn_$(head /dev/urandom | tr -dc 'a-zA-Z0-9' | fold -w 128 | head -n 1)"
		echo "$SERVER_CN" >SERVER_CN_GENERATED
		SERVER_NAME="server_$(head /dev/urandom | tr -dc 'a-zA-Z0-9' | fold -w 128 | head -n 1)"
